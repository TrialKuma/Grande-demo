...
test('v3.5 shields: cancelled enemy actions age shields and returning to zero never refreshes them',()=>{
  for(const partyIds of [['ric','apeilia','knibbs'],['knibbs','apeilia','ric']]){
    ...
    for(const h of s.heroes)assert.deepEqual(h.shieldLayers,[{amount:16,turns:1}]);
    s.boss.broken=true;s.boss.stagger=0;s.boss.controlImmune=0;endRound(s);
    for(const h of s.heroes)assert.deepEqual(h.shieldLayers,[]);
...
test('v3.5 saves: old shields get two rounds once; current layer duration cannot be forged',()=>{
  ...
  const migrated=normalizeSave(old);assert.ok(migrated);assert.equal(migrated.version,11);assert.equal(migrated.heroes[0].secondary,4);assert.deepEqual(migrated.heroes[0].shieldLayers,[{amount:31,turns:2}]);
...
test('v3.5 control: a two-charge lock skips one ordinary action without giving a break multiplier, then enforces immunity',()=>{
  const s=solo('qianxing','warden',['qianxing_lock']);s.loadouts.qianxing=['spike','repair','armor','lock'];
  cast(s,'qianxing','repair');const before=s.heroes[0].hp;
  cast(s,'qianxing','lock');assert.equal(s.boss.hardControl,1);assert.equal(s.boss.broken,false);assert.equal(s.heroes[0].resource,6);assert.equal(s.heroes[0].secondary,1);
...
test('v3.5 control: core and terminal rules cannot be bypassed by dispelling or locking',()=>{
  for(const boss of ['golem','final']){
    const s=solo('qianxing',boss,['qianxing_lock']);s.loadouts.qianxing=['spike','repair','armor','lock'];s.boss.hp=1;
    cast(s,'qianxing','repair');cast(s,'qianxing','spike');const before=structuredClone(s.boss);
    cast(s,'qianxing','lock');assert.equal(s.boss.core,before.core);assert.equal(s.boss.finale,before.finale);assert.equal(s.boss.hardControl,0);assert.equal(s.boss.coreHits,before.coreHits);assert.equal(s.boss.finaleHits,before.finaleHits);assert.equal(s.heroes[0].secondary,0);assert.equal(s.heroes[0].resource,10);
  }
});

test('v3.5 distinct treatments: first aid rescues immediately, suture heals a living target after danger',()=>{
  const s=createBattle('standard','duelist',{partyIds:['youmu','knibbs','apeilia'],upgrades:['youmu_suture']});s.loadouts.youmu=['scalpel','firstaid','suture','sterilize'];
...
test('v3.5 mind buffs: each ally consumes its own charge once; a recovery action cannot spend it',()=>{
  const s=createBattle('standard','duelist',{partyIds:['haart','knibbs','apeilia'],loadouts:{haart:['page','rest','anchor','relay'],knibbs:['shot','focus','breathe','cover']}});cast(s,'haart','rest');cast(s,'haart','anchor');
...
test('v3.5 all fourteen new upgrades change a concrete mechanic or unlock an independently selectable skill',()=>{
  ...
  const domain=solo('ric');domain.heroes[0].resource=-1;const before=effectiveAttributes(domain.boss,domain).strength;domain.upgrades.push('ric_erosion');assert.equal(effectiveAttributes(domain.boss,domain).strength,before-1);
