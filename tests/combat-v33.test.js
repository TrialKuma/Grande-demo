...
import {attackAttribute,defenseAttribute} from '../src/attributes.js';
...
const fixtureLoadouts={knibbs:['shot','scatter','breathe','cover'],apeilia:['blade','purify','eden','sentinel'],haart:['page','soothe','rest','relay']};
const fixture=(difficulty='standard',boss='golem',options={})=>createBattle(difficulty,boss,{...options,loadouts:{...fixtureLoadouts,...options.loadouts}});
...
const solo=(hero='knibbs',boss='golem',options={})=>fixture('standard',boss,{mode:'solo',partyIds:[hero],...options});
...
test('3.3: six-combo finishers require full payment and an alternating cycle funds either reward',()=>{
  for(const id of ['eden','sentinel']){
    const s=fixture('standard','tide'),h=heroOf(s,'apeilia');h.resource=5;
...
test('3.3: each added boss has four moves, a scene, and live telegraphs matching actual role-defense damage',()=>{
  ...
  for(const mode of ['party','solo'])for(const difficulty of Object.keys(DIFFICULTIES))for(const boss of added)for(const intent of BOSS_INTENTS[boss])for(const tactic of ['none','armor']){
    const s=fixture(difficulty,boss,{mode,partyIds:mode==='solo'?['qianxing']:['qianxing','haart','knibbs']});
    ...
    const info=intentInfo(s),before=s.heroes[0].hp;const spec=attackSpec(s);if(tactic==='armor'){s.heroes[0].secondary=1;cast(s,'qianxing','armor');}const expected=Math.max(0,Math.max(1,Math.round((spec.damage+attackAttribute(s.boss,spec.kind)-defenseAttribute(s.heroes[0],spec.kind))*DIFFICULTIES[difficulty].damage*(mode==='solo'?SOLO_RULES.bossDamage:1)))*(spec.hits||1)-s.heroes[0].shield);
    assert.ok(info.name&&info.desc&&info.icon,`${boss}/${intent}`);
    assert.match(info.desc,/(物理|魔法)伤害/);
    assert.equal(endRound(s).events.filter(e=>e.type==='boss'&&e.label===spec.name).reduce((sum,e)=>sum+(e.amounts?.qianxing||0),0),Math.min(before,expected),`${mode}/${difficulty}/${intent}/${tactic}`);
  }
...
test('3.3: tide counts hit segments across skills and turns, and filling retains its ordered effects without generic parry',()=>{
  ...
  s.boss.intent='tide_fill';heroOf(s,'knibbs').intuition=3;cast(s,'knibbs','cover');endRound(s);assert.equal(s.boss.waterLevel,3);
...
test('3.3: furnace opens after steam, grants real vulnerability, and any hit cools only while open',()=>{
  const s=fixture('standard','furnace');cast(s,'knibbs','shot');assert.equal(s.boss.heat,2);
...
test('3.3: orrery records whole attack skills once, supports and counterattacks cannot reset repetition',()=>{
  const s=fixture('standard','orrery');
  cast(s,'apeilia','purify');assert.equal(s.boss.prediction,0);assert.equal(s.boss.forecastSkill,'apeilia/purify');
  cast(s,'knibbs','breathe');assert.equal(s.boss.forecastSkill,'apeilia/purify');
  cast(s,'apeilia','purify');assert.equal(s.boss.prediction,1,'multiple hits count as one repeated skill');
  prepareResponse(s,'counter','knibbs');endRound(s);assert.equal(s.boss.forecastSkill,'apeilia/purify');assert.equal(s.boss.prediction,1);
  cast(s,'knibbs','shot');assert.equal(s.boss.prediction,0);assert.equal(s.boss.forecastSkill,'knibbs/shot');
...
test('3.3: arbiter checks actual empowered AP once per attack and clears judgement even after a break',()=>{
  const s=fixture('standard','arbiter',{upgrades:['apeilia_zero']});const h=heroOf(s,'apeilia');h.resource=6;h.lastKind='physical';
  cast(s,h.id,'sentinel');assert.equal(s.boss.violations,0,'one-AP empowered attack obeys light decree');
  cast(s,'apeilia','purify');assert.equal(s.boss.violations,1,'two segments are a single violation');
...
test('3.3: solo owns one hero, separate boss numbers, and five base AP plus capped carry',()=>{
  ...
  assert.equal(solo('missing').heroes[0].id,'knibbs');assert.equal(fixture().maxAp,6);assert.equal(fixture().boss.maxStagger,160);
...
test('3.3: single-attribute solo attacks legally complete the core and terminal still needs character defense',()=>{
  for(const hero of ['knibbs','haart']){
    ...
    if(hero==='knibbs'){cast(s,hero,'scatter');cast(s,hero,'shot');}else{cast(s,hero,'page');assert.equal(s.boss.coreHits,1);assert.ok(normalizeSave(s));for(let i=0;i<3;i++)cast(s,hero,'page');}
    assert.equal(s.mode,'victory');assert.equal(s.boss.reforms,0);
  }
  const s=solo('haart','final');s.boss.hp=1;cast(s,'haart','rest');cast(s,'haart','page');assert.equal(s.boss.finaleHits,0);
  cast(s,'haart','page');assert.equal(s.boss.finaleHits,2);assert.equal(s.boss.finalePhysical,0);assert.ok(normalizeSave(s));
...
test('3.3: current state and existing v5 saves roundtrip without changing damage or mode',()=>{
  for(const mode of ['party','solo'])for(const id of added){
    const s=fixture('standard',id,{mode,partyIds:mode==='solo'?['patch']:['patch','youmu','ric']});
    cast(s,'patch','bookward');cast(s,'patch','chargedslash');prepareResponse(s,'evade','patch');
  ...
  const old=fixture('standard','final');old.version=5;delete old.challengeMode;
  ...
  const restored=normalizeSave(old);assert.ok(restored);assert.equal(restored.version,11);assert.equal(restored.challengeMode,'party');assert.equal(restored.maxAp,6);assert.equal(restored.boss.maxHp,old.boss.maxHp);
});
...
test('3.3: switching dossiers preserves solo numbers and removes party-only core/terminal gates',()=>{
  ...
  for(const id of Object.keys(BOSSES).filter(id=>!BOSSES[id].isMinion&&!BOSSES[id].isSkirmish)){
    const markup=bossCodexView(s,id);assert.ok(markup.includes(`最大韧性 ${fixture(s.difficulty,id,{mode:'solo',partyIds:['haart']}).boss.maxStagger}`));assert.ok(markup.includes('基础 5 AP'));assert.match(markup,/最多.*2.*(未使用|保留)/);
    assert.ok(markup.includes(`>${fixture(s.difficulty,id,{mode:'solo',partyIds:['haart']}).boss.maxHp}</b>`));
...
for(const h of HEROES)test(`3.3: ${h.id} clears all ten standard solo encounters using a fixed four-slot build`,()=>{
  for(const boss of Object.keys(BOSSES).filter(id=>!BOSSES[id].isTutorial&&!BOSSES[id].isSkirmish&&!BOSSES[id].isMinion)){
    const r=runSolo(h.id,boss);assert.equal(r.result,'victory',`${h.id}/${boss}: ${r.result}`);assert.ok(r.hp>0);assert.equal(r.reforms,0);assert.ok(r.actions.length>0);assert.equal(r.loadout.length,4);assert.equal(new Set(r.loadout).size,4);
    if(boss==='final'){assert.ok(r.defenseAp>0);assert.equal(r.responseAp,0);};
  }
});

test('3.3: Haart can clear the zero-innate-stat final and growth is never silently granted to a baseline run',()=>{
 const base=runSolo('haart','final','standard',{includeState:true});
 assert.deepEqual(base.upgrades,[]);assert.equal(base.result,'victory');assert.equal(base.bossHp,0);
 assert.equal(base.loadout.length,4);assert.ok(base.hp>0);assert.equal(base.hp,base.state.heroes[0].hp);
 const upgrades=['haart_intercept','haart_echo'],loadout=[...base.loadout];
 const grown=runSolo('haart','final','standard',{loadout,upgrades,includeState:true});
 assert.deepEqual(grown.upgrades,upgrades);assert.deepEqual(grown.loadout,loadout);
 assert.equal(grown.result,'victory','The same legal build must also complete with explicitly supplied growth');
 assert.ok(grown.actions.length>0);assert.equal(grown.bossHp,0);
 // runSolo validates every committed playing frame; terminal states are deliberately not resumable saves.
 assert.ok(Number.isFinite(grown.hp)&&grown.hp>=0&&grown.hp<=grown.state.heroes[0].maxHp);
 assert.ok(Number.isFinite(grown.bossHp)&&grown.bossHp>=0&&grown.bossHp<=grown.state.boss.maxHp);
 if(grown.result==='defeat')assert.equal(grown.hp,0);
 else assert.ok(grown.hp>0);
});
