...
import {MANA_SKILLS} from '../src/mana-cycles.js';
...
test('mana integration: every hero starts with the canonical conversion and reclaim skills and no passive refund',()=>{
  for(const id of manaIds){
    ...
    const producer={haart:'page',qianxing:'spike',patch:'keyblade'}[id],p=skillPreview(s,id,producer);assert.equal(p.refund,0);assert.equal(p.secondaryAfter,2);assert.equal(p.resourceAfter,7);
    assert.ok(useSkill(s,id,producer).ok);assert.equal(h.resource,7);assert.equal(h.secondary,2);
...
test('mana integration: dynamic preview agrees with actual payment and direct damage across bosses, tiers, forms and upgrades',()=>{
  let checked=0;
  for(const id of manaIds)for(const bossId of Object.keys(BOSSES))for(const secondary of [0,1,2,3,6])for(const rewarded of [false,true])for(const base of MANA_SKILLS[id]){
...
test('mana integration: failed overflow or insufficient-secondary casts preserve all state',()=>{
  for(const id of manaIds){
    const s=setup(id),h=heroOf(s,id),producer={haart:'page',qianxing:'spike',patch:'keyblade'}[id];h.secondary=5;h.records=id==='patch'?5:0;
...
test('mana integration: emergency and low-stock reclamation do not retain the original attack, shield, heal or cooldown',()=>{
  for(const [id,producer,consumer] of [['haart','anchor','intercept'],['qianxing','armor','lock'],['patch','bookward','injunction']]){
    ...
    p=skillPreview(s,id,consumer);assert.equal(p.ap,1);assert.equal(p.damage,0);assert.ok(useSkill(s,id,consumer).ok);assert.equal(h.resource,2);assert.equal(h.secondary,0);assert.equal(s.boss.hp,initialBossHp);assert.equal(s.boss.hardControl,0);assert.equal(s.boss.weakened,0);assert.ok(!h.cooldowns[consumer]);
  }
});
...
test('mana integration: Haart grants an individual next-attack buff that survives utility and is consumed once',()=>{
  const s=setup('haart','golem',{upgrades:['haart_network']}),h=heroOf(s,'haart');equip(s,'haart','network');h.secondary=6;h.resource=1;
  assert.ok(useSkill(s,'haart','network').ok);assert.equal(h.resource,7);for(const ally of s.heroes)assert.equal(ally.attackBuff,40);
  const before=skillPreview(s,'knibbs','shot').damage;assert.ok(useSkill(s,'knibbs','breathe').ok);assert.equal(heroOf(s,'knibbs').attackBuff,40);assert.equal(skillPreview(s,'knibbs','shot').damage,before);
  assert.ok(useSkill(s,'knibbs','shot').ok);assert.equal(heroOf(s,'knibbs').attackBuff,0);assert.equal(heroOf(s,'apeilia').attackBuff,40);assert.equal(h.attackBuff,40);
});
...
test('mana integration: Patch archive upgrade cleanses every ally despite the shield being self-only',()=>{
  ...
  assert.equal(heroOf(s,'patch').shield,20);assert.equal(heroOf(s,'knibbs').shield,0);assert.equal(heroOf(s,'apeilia').shield,0);
});
...
test('mana integration: core and finale ignore hard control and strip effects but still consume records and return mana',()=>{
  for(const bossId of ['golem','final']){
    const s=setup('patch',bossId,{upgrades:['patch_injunction']}),h=heroOf(s,'patch');equip(s,'patch','injunction');h.secondary=3;h.records=3;h.resource=1;h.patchForm='record';
    s.boss.core=bossId==='golem';s.boss.finale=bossId==='final';s.boss.fog=5;s.boss.seals=3;
    assert.ok(useSkill(s,'patch','injunction').ok);assert.equal(h.resource,5);assert.equal(h.secondary,0);assert.equal(s.boss.hardControl,0);assert.equal(s.boss.fog,5);assert.equal(s.boss.seals,3);
  }
});

test('mana integration: v7 save retains mana, secondary stock and stance after both conversion and reclamation',()=>{
  for(const id of manaIds){
    const s=setup(id),producer={haart:'page',qianxing:'spike',patch:'bookward'}[id];assert.ok(useSkill(s,id,producer).ok);
    const restored=normalizeSave(structuredClone(s));assert.ok(restored,id);assert.equal(heroOf(restored,id).resource,7);assert.equal(heroOf(restored,id).secondary,2);
    const consumer={haart:'relay',qianxing:'pulse',patch:'chargedslash'}[id];assert.ok(useSkill(restored,id,consumer).ok);const again=normalizeSave(structuredClone(restored));assert.ok(again,id);
    assert.equal(heroOf(again,id).resource,9);assert.equal(heroOf(again,id).secondary,1);if(id==='patch')assert.equal(heroOf(again,id).patchForm,'record');
  }
});
