...
test('save: invalid top-level shapes, unsupported version, finished modes and unknown difficulty fail closed',()=>{
  ...
  invalid(s=>s.version=8);invalid(s=>s.mode='victory');invalid(s=>s.mode='defeat');invalid(s=>s.difficulty='missing');invalid(s=>s.difficulty='toString');invalid(s=>s.difficulty=['standard']);
});
...
test('save v2: a real v1 shape migrates to golem with new passives and no prepared response',()=>{
  ...
  assert.equal(restored.version,7);assert.equal(restored.boss.id,'golem');
...
test('save v3: a captured v2 duelist response migrates without losing resources or prepared tactics',()=>{
  ...
  const restored=normalizeSave(value);assert.ok(restored);assert.equal(restored.version,7);
