...
test('Ric full +/-10 range, voluntary reversal and automatic two-point return retain zero-cross effects',()=>{
 ...
 h.resource=-2;absorbShield(h,h.shield);prepareResponse(s,'evade','patch');endRound(s);assert.equal(h.resource,0);assert.equal(Object.is(h.resource,-0),false);assert.equal(h.shield,10);
});
...
test('Youmu sacrifices full health to captain form with five actions, two-round duration and a finite debuff',()=>{
 const s=battle(),h=heroOf(s,'youmu');cast(s,'youmu','bloodoath');assert.equal(h.hp,66);assert.equal(h.tauntTurns,2);assert.equal(h.youmuForm,'captain');assert.equal(h.captainUsed,true);
