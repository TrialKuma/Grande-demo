...
import {createBattle,heroOf,useSkill,endRound,HEROES,SKILLS,skillPreview,enemyTargets,activeSkills} from '../src/combat.js';
...
const cast=(state,hero,id)=>{if(!activeSkills(state,hero).some(s=>s.id===id))state.loadouts[hero][3]=id;assert.equal(useSkill(state,hero,id).ok,true,`${hero}/${id}`);};
...
test('battle skills and portraits omit teaching lists while status details and the journal retain guidance',()=>{
 for(const hero of HEROES){
  ...
  for(const skill of SKILLS[hero.id]){
   ...
   assert.match(html,/AP 消耗/);assert.doesNotMatch(html,/undefined|NaN/);
  }
  ...
  assert.match(tooltipView(state,'status',hero.id,'passive'),heroOf(state,hero.id).secondaryName?/mana-recovery/:/tooltip-mechanism/);assert.doesNotMatch(tooltipView(state,'status',hero.id,'passive'),/tooltip-notes/);
...
test('prepared enemy effects appear on their own caster, name the target and expire after real enemy actions',()=>{
 ...
 // This ownership fixture begins with a prepared mindline; conversion is covered by the mana-loop tests.
 heroOf(state,'haart').secondary=1;heroOf(state,'knibbs').intuition=3;
 cast(state,'knibbs','cover');cast(state,'haart','soothe');cast(state,'patch','bookward');cast(state,'patch','chargedslash');
 ...
 assert.match(words(tooltipView(state,'status','haart','confusion')),/力量与智力各降低 16/);
 assert.match(words(tooltipView(state,'status','patch','recordedIntent')),/力量与智力各降低 8/);
 endRound(state);
 for(const [hero,id] of [['knibbs','cover'],['haart','confusion'],['patch','recordedIntent']])assert.equal(token(state,hero,id),'');
});

test('three intuition layers prepare a counter without adding per-hit intuition damage or consuming ammunition',()=>{
 ...
 assert.match(preview,/基本物理伤害 55/);assert.doesNotMatch(preview,/\(\+12\)/);assert.match(preview,/预计削韧 30/);
 cast(state,'knibbs','cover');assert.equal(h.intuition,0);
 assert.match(words(tooltipView(state,'status','knibbs','cover')),/55 基础物理 \/ 削韧 30/);
 assert.match(words(tooltipView(state,'hero','knibbs')),/直感截击/);
});
...
test('evasion and reactive bandaging show one-use limits and vanish even if the protected hero was not attacked',()=>{
 ...
 assert.match(words(token(state,'youmu','surgicalReady')),/手术准备 1/);
});
...
test('captain self-empowerment uses the same real one-use buff as team empowerment and clears on attack',()=>{
 ...
 assert.match(words(token(state,h.id,'attackBuff')),/战术增幅 1次·2轮/);
 assert.match(words(token(state,h.id,'captain')),/船长 2轮/);assert.match(words(token(state,h.id,'taunt')),/嘲讽 2轮/);
 assert.match(words(tooltipView(state,'status',h.id,'captain')),/力量 \+5.*智力 -5.*敏捷 \+9.*意志 \+2/);
...
test('temporary reaction charges, independent shields and bad states each have readable names and durations',()=>{
 ...
 const ric=heroOf(state,'ric');ric.resource=-2;cast(state,'ric','shelter');
 for(const [id,label] of [['edge','剑势 2次'],['grace','余响 1次'],['verdict','清账 1次']])assert.ok(words(token(state,ric.id,id)).includes(label));
 cast(state,'qianxing','repair');cast(state,'qianxing','armor');
 assert.match(words(token(state,'qianxing','reflect')),/钉刺 1次/);
 assert.match(words(token(state,'qianxing','protection')),/防护姿态 1轮/);
 assert.match(words(token(state,'qianxing','shield')),/护盾36 首批2轮/);
...
test('skill hovers retain exact transformation, interrupt, AOE, payment and recovery facts without teaching lists',()=>{
 ...
 assert.match(words(tooltipView(state,'skill','youmu','bloodoath')),/主动将生命降至最大值的 40%.*每战一次.*敏捷 \+9.*虚脱两轮/);
 ...
 assert.match(words(emergency),/只完成准备，不附带攻击、防护或治疗/);assert.doesNotMatch(emergency,/tooltip-notes/);
 const aoe=createBattle('standard','patrol');heroOf(aoe,'apeilia').resource=6;const preview=skillPreview(aoe,'apeilia','sentinel'),tip=words(tooltipView(aoe,'skill','apeilia','sentinel'));
 assert.match(tip,/魔法攻击 · 全体/);assert.match(tip,/基本魔法伤害/);
 for(const enemy of enemyTargets(aoe))assert.ok(!tip.includes(`${enemy.name} · 预计扣血`));
});
...
test('reward names remain linked while full reward conditions stay on the character portrait',()=>{
 ...
 assert.match(skill,/装填 · 稳手/);assert.doesNotMatch(skill,/tooltip-notes/);assert.match(hero,/tooltip-notes/);
 heroOf(state,'knibbs').intuition=2;
 assert.match(words(tooltipView(state,'skill','knibbs','breathe')),/力量 \+10、智力 \+10/);
});
