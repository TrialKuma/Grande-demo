...
const equip=(s,id,skill)=>{if(!s.loadouts[id].includes(skill))s.loadouts[id][3]=skill;};
const card=(s,id,skill)=>{equip(s,id,skill);return cards(s,id).find(b=>b.attrs.includes(`data-skill="${skill}"`));};
const cast=(s,id,skill)=>{equip(s,id,skill);return useSkill(s,id,skill);};
...
test('all seven four-slot rows show the actual attack type; restorative and control slots say support',()=>{
 for(const hero of HEROES){
  const s=make(hero.id),before=structuredClone(s),visible=cards(s,hero.id);assert.equal(visible.length,4);
...
test('each mana loop shows its distinct capacity and separates dedicated preparation from passive recovery',()=>{
 for(const [id,convert,cashout,name,cost,gain,spent,cap,refund]of [['haart','rest','soothe','念线',8,4,1,4,3],['qianxing','repair','pulse','充能',9,3,1,3,4],['patch','bookward','chargedslash','记录',4,3,1,10,3]]){
  const s=make(id),h=heroOf(s,id),before=words(tooltipView(s,'skill',id,convert));
  assert.doesNotMatch(before,/施放时消耗|生成 1 点/);assert.match(tooltipView(s,'skill',id,convert),new RegExp(`aria-label="魔力 10 → ${10-cost}"`));assert.equal(cast(s,id,convert).ok,true);
  assert.equal(h.maxSecondary,cap);assert.equal(h.secondary,gain);assert.equal(h.resource,10-cost);
  const html=render(s),tip=words(tooltipView(s,'skill',id,cashout));
  assert.ok(html.includes(`aria-label="${name} ${gain}/${cap}，悬浮查看回魔被动"`));assert.ok(html.includes(`aria-label="${h.short}的魔力"`));
  const detail=tooltipView(s,'skill',id,cashout);assert.ok(detail.includes(`aria-label="${name} ${gain} → ${gain-spent}"`));assert.ok(detail.includes(`aria-label="魔力 ${10-cost} → ${10-cost+refund}"`));
  assert.match(tip,/魔力 · 被动回魔/);assert.match(tip,new RegExp(`消耗${name}后回魔，每次行动仅一次`));
  assert.doesNotMatch(skillExplanation(s,id,cashout).cost,/返还|回魔/);
  assert.doesNotMatch(tip,/必须先拥有足额魔力|每轮第一次攻击会额外回复|受击会增加/);
  assert.equal(cast(s,id,cashout).ok,true);assert.equal(h.secondary,gain-spent);assert.equal(h.resource,10-cost+refund);
...
test('each bulk conversion spends two AP on preparation and discloses its own mana-to-stock ratio',()=>{
 for(const [id,skill,cost,gain]of [['haart','rest',8,4],['qianxing','repair',9,3],['patch','collate',8,6]]){
  ...
  assert.ok(tooltipView(s,'skill',id,skill).includes(`aria-label="${h.secondaryName} 0 → ${gain}"`));
  ...
  assert.equal(cast(s,id,skill).ok,true);assert.equal(s.ap,4);assert.equal(h.resource,10-cost);assert.equal(h.secondary,gain);
 }
});
...
test('Patch stance changes damage type while a stockpile never removes its fixed one-record cashout',()=>{
 ...
 assert.equal(cast(s,'patch','bookward').ok,true);
 ...
 assert.match(active.body,/充能斩 · 镜反/);assert.match(tooltipView(s,'skill','patch','chargedslash'),/aria-label="记录 6 → 5"/);
 assert.match(tooltipView(s,'skill','patch','chargedslash'),/aria-label="魔力 2 → 5"/);assert.doesNotMatch(tip,/充能斩 · 反证|消耗 3 点记录/);
});
...
test('Patch gate previews six-to-ten consumed records and one stance-dependent passive payout',()=>{
 for(const form of ['observe','record'])for(const records of [6,8,10]){
  const s=createBattle('standard','warden',{partyIds:['patch','knibbs','ric'],upgrades:['patch_revelation'],loadouts:{patch:['keyblade','bookward','chargedslash','revelation']}}),h=heroOf(s,'patch');
  ...
  assert.ok(tip.includes(`攻击段数 ${records} 段`));assert.ok(tooltipView(s,'skill','patch','revelation').includes(`aria-label="魔力 0 → ${refund}"`));
  assert.ok(tooltipView(s,'skill','patch','revelation').includes(`aria-label="记录 ${records} → 0"`));assert.equal(preview.hits,records);assert.equal(preview.refund,refund);
  if(form==='record'&&records>=8)assert.match(tip,/封锁敌人的下一次普通行动/);
  else assert.doesNotMatch(tip,/封锁敌人的下一次普通行动/);
  assert.equal(cast(s,'patch','revelation').ok,true);assert.equal(h.secondary,0);assert.equal(h.resource,refund);
 }
});

test('low-stock cashouts retain their attack while only dedicated preparation may become emergency preparation',()=>{
 ...
 assert.match(recovery.body,/聚焦光束/);assert.match(recovery.body,/data-skill-type="magic"/);assert.doesNotMatch(recovery.attrs,/empowered/);
 assert.match(recovery.body,/is-insufficient[^>]*data-resource="charge" data-amount="-2"/);assert.doesNotMatch(tip,/逆向提炼|当前不可施放/);
 h.secondary=0;const emergency=card(s,'qianxing','repair');
 assert.match(emergency.body,/应急准备/);assert.match(emergency.body,/data-skill-type="support"/);assert.doesNotMatch(emergency.attrs,/empowered/);
 assert.match(words(tooltipView(s,'skill','qianxing','repair')),/只完成准备，不附带攻击、防护或治疗/);
});
...
test('full stock exposes a powerful but less efficient cashout while fixed small cashout stays available',()=>{
 ...
 assert.equal(cast(s,'qianxing','repair').ok,true);
 ...
 assert.match(card(s,'qianxing','beam').body,/超临界/);assert.match(words(tooltipView(s,'skill','qianxing','beam')),/敏捷 -8、智力 -8，持续两轮/);
...
test('auxiliary cashouts lead with their tactical effect while passive recovery stays in the detailed preview',()=>{
 const h=make('haart');assert.equal(cast(h,'haart','rest').ok,true);
 assert.match(words(card(h,'haart','soothe').body),/杀意改写 · 转向、力\/智 −16/);assert.doesNotMatch(words(card(h,'haart','soothe').body),/回魔/);
 assert.match(words(tooltipView(h,'skill','haart','soothe')),/下一次单体攻击转向另一名敌人/);
 assert.match(words(card(h,'haart','anchor').body),/辅助.*全队 力量 \+9、智力 \+9/);
 const p=make('patch');assert.match(card(p,'patch','collate').body,/data-resource="record" data-amount="6"/);
 const q=createBattle('standard','warden',{partyIds:['qianxing','knibbs','ric'],upgrades:['qianxing_lock'],loadouts:{qianxing:['spike','beam','armor','lock']}});
 assert.equal(cast(q,'qianxing','repair').ok,true);
 assert.match(words(card(q,'qianxing','lock').body),/辅助 单体.*封锁行动 · 驱散 3 层/);assert.doesNotMatch(words(card(q,'qianxing','lock').body),/回魔/);
 assert.match(tooltipView(q,'skill','qianxing','lock'),/aria-label="魔力 1 → 6"/);
});

test('shield icons explain separate remaining batches and disappear when actual settlement expires them',()=>{
 const s=make('qianxing'),h=heroOf(s,'qianxing');
 assert.equal(cast(s,'qianxing','repair').ok,true);assert.equal(cast(s,'qianxing','armor').ok,true);s.boss.broken=true;endRound(s);
 assert.deepEqual(h.shieldLayers,[{amount:36,turns:1}]);
 assert.equal(cast(s,'qianxing','spike').ok,true);
 assert.equal(cast(s,'qianxing','armor').ok,true);
...
test('first aid describes and performs single-target revival; suture is delayed and cannot revive',()=>{
 const s=createBattle('standard','warden',{partyIds:['youmu','knibbs','ric'],upgrades:['youmu_suture'],loadouts:{youmu:['scalpel','surgery','firstaid','suture']}}),youmu=heroOf(s,'youmu'),patient=heroOf(s,'knibbs'),other=heroOf(s,'ric');
 ...
 assert.equal(cast(s,'youmu','firstaid').ok,true);assert.equal(patient.hp,30);assert.equal(patient.resonance,2);assert.equal(other.hp,80);
 ...
 assert.equal(cast(s,'youmu','suture').ok,true);assert.equal(patient.hp,before);assert.equal(patient.regenTurns,2);assert.match(statusBadges(s,patient),/data-detail="regen"/);
...
test('blood oath is explained and usable at full health, with a live taunt icon and captain portrait',()=>{
 ...
 assert.equal(cast(s,'youmu','bloodoath').ok,true);assert.match(render(s),/portrait youmu_inner/);assert.match(statusBadges(s,h),/data-detail="taunt"/);
...
test('one-use attack buffs are visible on every recipient and their real consumption removes the icon',()=>{
 const s=make('haart');assert.equal(cast(s,'haart','rest').ok,true);assert.equal(cast(s,'haart','anchor').ok,true);
 for(const h of s.heroes){assert.match(statusBadges(s,h),/data-detail="attackBuff"/);assert.match(words(tooltipView(s,'status',h.id,'attackBuff')),/下一项主动伤害技能的每一段/);}
 assert.match(card(s,'knibbs','shot').attrs,/empowered/);assert.equal(cast(s,'knibbs','shot').ok,true);
...
test('hard control explains the cancelled enemy action without showing retired response buttons',()=>{
 const s=createBattle('standard','warden',{partyIds:['qianxing','knibbs','ric'],upgrades:['qianxing_lock'],loadouts:{qianxing:['spike','beam','armor','lock']}});
 assert.equal(cast(s,'qianxing','repair').ok,true);assert.equal(cast(s,'qianxing','lock').ok,true);
...
test('all 35 growth rewards are listed automatically, including every skill unlock, in journal and camp',()=>{
 ...
 for(const h of HEROES){
  ...
  assert.equal((camp.match(/data-loadout-slot=/g)||[]).length,4);assert.equal((camp.match(/data-equip-skill=/g)||[]).length,SKILLS[h.id].length);
...
test('the upgraded book ward separates a self-only two-round shield from its actual party cleanse',()=>{
 ...
 assert.equal(cast(s,'patch','bookward').ok,true);assert.deepEqual(s.heroes.map(h=>h.resonance),[2,2,2]);assert.deepEqual(s.heroes.map(h=>h.shield),[8,0,0]);
});
...
test('transplantation is an auxiliary form of surgery with no active damage or new wound',()=>{
 ...
 assert.equal(cast(s,'youmu','scalpel').ok,true);assert.equal(cast(s,'youmu','surgery').ok,true);
...
test('the manual and portraits state the current two-resource economy without obsolete recovery promises',()=>{
 ...
 assert.match(texts,/念线/);assert.match(texts,/充能/);assert.match(texts,/记录/);assert.match(texts,/付出生命，请游墓接管/);
});
