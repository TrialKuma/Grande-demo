...
362 段中文对白和旁白通过第三方 [edge-tts 客户端](https://github.com/rany2/edge-tts)访问 Microsoft Edge Read Aloud 在线神经语音服务生成。成品为合成 MP3，游戏与独立 HTML 播放本地音频；生成客户端不是微软官方 SDK，也不随游戏作为运行依赖。选声配置、来源记录及生成方式见 `story-and-voice.md` 和 `public/voices/manifest.json`。
