...
document.addEventListener('keydown',e=>{
  ...
  if(!modal&&screen!=='art-review'&&[' ','escape','h','q','w','e','1','2','3','r','v','b'].includes(key))e.preventDefault();if(e.repeat)return;
  ...
  const index=['q','w','e','r'].indexOf(key);if(index>=0){const skill=activeSkills(state,state.selected)[index];if(!skill)return;lastSkill={owner:state.selected,id:skill.id};audio.unlock();perform(()=>useSkill(state,state.selected,lastSkill.id));return;}
