...
function campView(run,{partySlot=0,skillSlot=0}={}){
 ...
 ${isLearningRun(run)?`<div class="learning-summary"><strong>本次学会</strong>${run.lastLearning?.length?run.lastLearning.map(item=>`<span>${hero(item.heroId).short} · ${esc(SKILLS[item.heroId].find(s=>s.id===item.skillId)?.name)}</span>`).join(''):formalLearning(run)?'<span>正式远征中的同伴至少掌握四招。第五招可在间章操练、途中实战或个人演练中学会。</span>':'<span>入门阶段逐步熟悉技能；进入正式远征前，同伴会补齐至少四项基础技能。</span>'}</div>`:''}
